...
{
    public const double DefaultExposureTimeUs = 10_000;
    ...
    public double ExposureTimeUs { get; set; } = DefaultExposureTimeUs;
    ...
    public bool EnableOriginalImageFileNameWatermark { get; set; } = true;
    public int OriginalImageWatermarkFontSize { get; set; } = CameraImageWatermarkOptions.DefaultFontSize;
    public string OriginalImageWatermarkColor { get; set; } = CameraImageWatermarkOptions.DefaultColor;
    public CameraImageWatermarkPosition OriginalImageWatermarkPosition { get; set; } = CameraImageWatermarkPosition.BottomRight;
    ...
    public CameraProfile Normalize(int ordinal)
    {
        return new CameraProfile
        {
            ...
            AutoExposure = AutoExposure,
            ExposureTimeUs = double.IsFinite(ExposureTimeUs) && ExposureTimeUs > 0
                ? ExposureTimeUs
            ...
            EnableOriginalImageFileNameWatermark = EnableOriginalImageFileNameWatermark,
            OriginalImageWatermarkFontSize = Math.Clamp(OriginalImageWatermarkFontSize, 12, 128),
            OriginalImageWatermarkColor = CameraImageWatermarkOptions.TryNormalizeColor(OriginalImageWatermarkColor, out var watermarkColor)
                ? watermarkColor
                : CameraImageWatermarkOptions.DefaultColor,
            OriginalImageWatermarkPosition = Enum.IsDefined(OriginalImageWatermarkPosition)
                ? OriginalImageWatermarkPosition
                : CameraImageWatermarkPosition.BottomRight,
            ExtensionData = CloneExtensionData(ExtensionData)
        };
    }
    ...
    public CameraOpenOptions BuildOpenOptions(double? targetFps = null)
    {
        return new CameraOpenOptions(
                ...
                MirrorMode,
                AutoExposure,
                AutoExposure ? null : ExposureTimeUs)
            .Normalize();
    }
