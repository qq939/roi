...
public sealed class WindowsHikCameraProvider : ICameraProvider
{
    ...
    public ICameraSession Open(CameraOpenOptions options)
    {
        ...
        try
        {
            ...
            if (normalized.ConfigureDevice)
            {
                ConfigureDevice(
                    device,
                    normalized.TargetFps,
                    normalized.TriggerMode,
                    normalized.AutoExposure,
                    normalized.ExposureTimeUs);
                CameraDiagnostics.Info("hik-provider", $"Device parameters configured for {BuildDisplayName(deviceInfo)}.");
            }
            else
            {
                CameraDiagnostics.Info("hik-provider", $"Device parameters left unchanged for {BuildDisplayName(deviceInfo)}.");
            }
        ...
        double? exposureTimeUs)
    {
        ...
        ConfigureExposure(parameters, autoExposure, exposureTimeUs);
        LogConfiguredTiming(parameters, targetFps, triggerMode, autoExposure, exposureTimeUs);
    }
    ...
    private static void ConfigureExposure(
        ...
        double? exposureTimeUs)
    {
        if (!autoExposure.HasValue)
        {
            return;
        }

        var autoMode = autoExposure.Value ? "Continuous" : "Off";
        if (!TrySetEnumValue(parameters, "ExposureAuto", autoMode, logFailures: false))
        {
            CameraDiagnostics.Info("hik-provider", $"ExposureAuto={autoMode} was not applied; node is unavailable or read-only.");
        }
        ...
        {
            throw new InvalidOperationException("Manual exposure is enabled, but ExposureTimeUs was not configured.");
        }

        var settings = ReadExposureSettings(parameters);
        if (!settings.IsSupported || settings.MinimumExposureTimeUs is not { } minimum || settings.MaximumExposureTimeUs is not { } maximum)
        {
            throw new InvalidOperationException($"Hik camera does not expose writable ExposureTime. {settings.Message}");
        }

        if (exposureTimeUs.Value < minimum || exposureTimeUs.Value > maximum)
        {
            throw new ArgumentOutOfRangeException(
                nameof(exposureTimeUs),
                exposureTimeUs,
                $"ExposureTimeUs must be within {minimum:0.##} to {maximum:0.##} us.");
        }

        EnsureSuccess(
            parameters.SetFloatValue("ExposureTime", (float)exposureTimeUs.Value),
            "Set Hik ExposureTime");
        var applied = ReadExposureSettings(parameters);
        CameraDiagnostics.Info(
            "hik-provider",
            $"Configured exposure. Auto=Off, RequestedUs={exposureTimeUs.Value:0.##}, AppliedUs={FormatDiagnosticFloat(applied.ExposureTimeUs ?? 0)}, RangeUs={minimum:0.##}-{maximum:0.##}");
    }

    private static CameraExposureSettings ReadExposureSettings(IParameters? parameters)
    {
        if (parameters == null)
        {
            return CameraExposureSettings.Unsupported("Hik camera parameters are unavailable.");
        }

        try
        {
            IFloatValue value = null!;
            var result = parameters.GetFloatValue("ExposureTime", out value);
            if (result != 0 || value == null)
            {
                return CameraExposureSettings.Unsupported(
                    $"ExposureTime is unavailable ({ResolveErrorName(result)} 0x{unchecked((uint)result):X8}).");
            }

            var autoSymbol = TryReadCurrentEnumSymbol(parameters, "ExposureAuto");
            bool? autoExposure = string.IsNullOrWhiteSpace(autoSymbol)
                ? null
                : !autoSymbol.Equals("Off", StringComparison.OrdinalIgnoreCase);
            return new CameraExposureSettings(
                true,
                autoExposure,
                value.CurValue,
                value.Min,
        ...
        {
            return CameraExposureSettings.Unsupported($"Read ExposureTime failed: {ex.Message}");
        }
    }

    private static void LogConfiguredTiming(
        IParameters parameters,
        double targetFps,
        CameraTriggerMode triggerMode,
        bool? autoExposure,
        double? exposureTimeUs)
    {
        ...
        CameraDiagnostics.Info(
            "hik-provider",
            $"Configured timing. TriggerMode={triggerMode}, RequestedFps={targetFps:F2}, AcquisitionFrameRate={FormatDiagnosticFloat(acquisitionFrameRate)}, ResultingFrameRate={FormatDiagnosticFloat(resultingFrameRate)}, AutoExposure={autoExposure?.ToString() ?? "unchanged"}, RequestedExposureTimeUs={FormatDiagnosticFloat(exposureTimeUs ?? 0)}, ExposureTimeUs={FormatDiagnosticFloat(exposureTime)}");
    }
    ...
    private sealed class HikCameraSession : ICameraSession, ICameraFrameCallbackSource, ICameraExposureSettingsSource
    {
        ...
        public CameraExposureSettings ReadExposureSettings() =>
            WindowsHikCameraProvider.ReadExposureSettings(_device.Parameters);
